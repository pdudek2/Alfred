...
export function mergePreviewUrlCandidates(
  ...
  for (const candidate of incomingCandidates) {
    ...
    const latestCandidate = candidate.lastSeenAt >= existingCandidate.lastSeenAt ? candidate : existingCandidate;
    ...
    candidatesByKey.set(key, {
      ...
      sessionTitle: latestCandidate.sessionTitle,
      firstSeenAt: Math.min(existingCandidate.firstSeenAt, candidate.firstSeenAt),
      lastSeenAt: Math.max(existingCandidate.lastSeenAt, candidate.lastSeenAt),
    });
  }
